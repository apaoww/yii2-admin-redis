...
- Enh: Custom side menu via `apaoww\AdminRedis\Module::items` (mdmunir).
...
- Chg: Can customize return of `apaoww\AdminRedis\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `apaoww\AdminRedis\components\BizRule` (mdmunir).
...
- Chg: Remove `apaoww\AdminRedis\Module::allowActions`. Set access control directly with `apaoww\AdminRedis\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `apaoww\AdminRedis\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
